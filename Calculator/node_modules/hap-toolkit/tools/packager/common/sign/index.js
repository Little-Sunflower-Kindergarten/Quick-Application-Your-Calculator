 ...
 * 加签名zip文件
 * @param options
 * @param options.content ZIP文件的内容
 * @param options.files 文件列表，每个元素拥有：name, hash
 * @param prikey
 * @param certpem
 */
function signZip(options, prikey, certpem) {
  var cert = Buffer.from(_base2.default.unarmor(certpem));
...
function makeSignChunk(options, sign, prikey, pubkey, cert) {
  ...
  // 将public.pem转化为der
  var pubbuf = Buffer.from(_base2.default.unarmor(pubkey));
  var signBlock = {
...
function signFiles(filehashs, prikey) {
  var chunk = {
    ...
    // name hash
    var namehash = _crc2.default.digest(item.name.toString());
