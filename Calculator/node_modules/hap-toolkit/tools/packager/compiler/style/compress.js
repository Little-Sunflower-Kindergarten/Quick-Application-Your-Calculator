...
// CSS后代选择器全写-缩写对照表
var cssDescSelectorList = [{ newN: 't', oldN: 'type', newV: ['d', 'a', 't', 'u', 'p', 'pe'], oldV: ['descendant', 'attribute', 'tag', 'universal', 'pseudo', 'pseudo-element'] }, { newN: 'n', oldN: 'name' }, { newN: 'i', oldN: 'ignoreCase' }, { newN: 'a', oldN: 'action' }, { newN: 'v', oldN: 'value' }];
...
function compressCssAttr(jsonStyle) {
  for (var selector in jsonStyle) {
    var cssAttrObj = jsonStyle[selector];
    if (selector !== '@KEYFRAMES' && (typeof cssAttrObj === 'undefined' ? 'undefined' : _typeof(cssAttrObj)) === 'object') {
      for (var cssAttrName in cssAttrObj) {
        if (cssAttrMap[cssAttrName]) {
          var cssAttrAbbrName = cssAttrMap[cssAttrName];
